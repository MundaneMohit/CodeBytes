...
DataCleanser <- function(Raw_Data)
{
  ...
  Raw_Data <- Raw_Data[which(!is.na(Raw_Data$CUSTOMER_TENURE)),] 
...
DataEngineer <- function(Raw_Data)
{
#### Add multiple levels for PRIMSIC. Replace missing with neutral value (Eg. "NNNN")####
# Convert to Character
Raw_Data$PRIMSIC_COMBINED <- as.character(Raw_Data$PRIMSIC_COMBINED)
# Padding left
Raw_Data$PRIMSIC_COMBINED <- str_pad(Raw_Data$PRIMSIC_COMBINED,4,c("left"), pad = 0)
# Replace NAs with neutral value
Raw_Data[which(is.na(Raw_Data$PRIMSIC_COMBINED)), "PRIMSIC_COMBINED"] <- "NNNN"
# Add multiple levels for PRIMSIC
Raw_Data$PRIMSIC_1 <- substr(Raw_Data$PRIMSIC_COMBINED,0,2)
Raw_Data$PRIMSIC_2 <- substr(Raw_Data$PRIMSIC_COMBINED,2,3)
Raw_Data$PRIMSIC_3 <- substr(Raw_Data$PRIMSIC_COMBINED,3,4)

# Data type conversion
Raw_Data$PRIMSIC_COMBINED <- as.factor(Raw_Data$PRIMSIC_COMBINED)

#### Adding new Contract variable ####
Raw_Data$CONTRACT_FLAG <- ifelse(Raw_Data$MONTHS_FROM_END == 9999, "NO_CONTRACT",
                                ifelse(Raw_Data$MONTHS_FROM_END >= 0, "IN_CONTRACT",
                                       ifelse(Raw_Data$MONTHS_FROM_END < 0, "POST_CONTRACT","ERR")))
Raw_Data$CONTRACT_FLAG <- as.factor(Raw_Data$CONTRACT_FLAG)

#### RETAIN OLD CONTRACT VARIABLES AS WELL ####
Raw_Data$IN_CONTRACT <- Raw_Data$MONTHS_FROM_END < 0
Raw_Data$POST_CONTRACT <- Raw_Data$MONTHS_TO_END < 0                          
Raw_Data$NO_CONTRACT <- Raw_Data$MONTHS_FROM_END == 9999   

return(Raw_Data)

}

HSI_RAW <- DataEngineer(HSI_RAW)
HSI_VAL <- DataEngineer(HSI_VAL)

table(HSI_RAW$HAZARD)
############################ TRAINING SET BINNING##########################################
...
HSI_VAL$CUSTEN_10BINS <- cut(HSI_VAL$CUSTOMER_TENURE, c(0,cutpoints,max(HSI_VAL$CUSTOMER_TENURE)), lab = c("<8", "8-14", "15-26", "27-53", "54-66", "67-89", "90-132", "133-266", "267-374", ">374"))
...
HSI_VAL$PRODTEN_BINS <- cut(HSI_VAL$PRODUCT_TENURE, c(0,cutpoints,max(HSI_VAL$PRODUCT_TENURE)), lab = c("<6", "6-14", "15-32", "33-36", "37-43", "44-47", "48-52", "53-78", ">78"))
...
HSI_VAL$MTHFEND_BINS <- cut(HSI_VAL$MONTHS_FROM_END, c(min(HSI_VAL$MONTHS_FROM_END)-1,cutpoints,max(HSI_VAL$MONTHS_FROM_END)), lab = c("<-22", "-21 to -2", "-1 to 1397", "NO_CONTRACT"))
...
HSI_VAL$MTHTOEND_BINS <- cut(HSI_VAL$MONTHS_TO_END, c(min(HSI_VAL$MONTHS_TO_END)-1,cutpoints,342, max(HSI_VAL$MONTHS_TO_END)), lab = c("< 1", "2 to 21", "22 to 33", "34 to 342", "NO_CONTRACT"))
...
HSI_VAL$MTHFSTRT<- cut(HSI_VAL$MONTHS_FROM_START, c(min(HSI_VAL$MONTHS_FROM_START)-1,cutpoints, max(HSI_VAL$MONTHS_FROM_START)), lab = c("<6", "7 to 20", "21 to 41", "42 to 159", "NO_CONTRACT"))
...
HSI_VAL$CALL_COUNT_BINNED <- cut(HSI_VAL$CALL_COUNT, c(-1,cutpoints,max(HSI_VAL$CUSTOMER_TENURE)), lab = c("0", "1-2", ">2"))
...
HSI_VAL$CALL_DUR_BINNED <- cut(HSI_VAL$CALL_DURATION, c(-1,0,cutpoints,max(HSI_VAL$CALL_DURATION)), lab = c("0", "1-7", "8-665", "666-1495", ">1495"))
...
HSI_VAL$AGENTTIME_BINNED <- cut(HSI_VAL$TIME_WITH_AGENT, c(-2,-1,0,73,max(HSI_VAL$TIME_WITH_AGENT)), lab = c("<0", "0", "1-73", ">73"))
...
HSI_VAL$ANSHOLD_BINNED <- cut(HSI_VAL$ANSHOLDTIME, c(-2,-1,0,5,max(HSI_VAL$ANSHOLDTIME)), lab = c("<0", "0", "1-5", ">5"))
...
HSI_VAL$QUEUE_BINNED <- cut(HSI_VAL$QUEUETIME, c(-2, -1, 0,36,max(HSI_VAL$QUEUETIME)), lab = c("<0", "0", "1-36", ">36"))
...
HSI_VAL.Sample.Test <- CreateDataSample(HSI_VAL[which(HSI_VAL$REGION != "UNKNOWN"),], 60000, 111)
...
HSI.Prediction <- predictSurvProb(HSI.CoxModel,HSI_VAL.Sample.Test,  c(1,2,3,4,5,6,12))

summary(HSI_VAL.Sample.Test$CUSTOMER_TENURE)
...
HSI.final_merge<- merge(HSI_VAL.Sample.Test, y = HSI.Prediction, by="row.names")
...
size <- 6000
MAT2 <- table(HSI.final_merge[which(HAZARD == 1 & DUR <= 3 & SNO <= size),]$SNO,HSI.final_merge[which(HAZARD == 1 & SNO <= size),]$DUR)
head(MAT2)
MAT2 <-   data.frame(ROW = c(1:size), CHURN = (MAT2[,1]+MAT2[,2]))
head(MAT2)
...
write.csv(HSI.final_merge,"C:/Users/AB37179/Documents/My Projects/HSI Predictive Churn Analysis/Survival Analysis/Model Results/16_MODEL_TEST_VALIDATION.csv", row.names = FALSE)
